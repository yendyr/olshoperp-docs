...
test.describe('ETM-15783 — Skip Wave Process Advanced Filter: Processing Start & End', () => {
  test('[@ETM-15783] Verify Processing Start & End filter with datetime operators on merdian.olshoperp.com', async ({ browser }) => {
    ...
    const setupProcStartCondition = async (conditionVal: string, value1?: string, value2?: string) => {
      ...
      await dataSelect.selectOption('4'); // Processing Start & End
      await page.waitForTimeout(1000);
    ...
    console.log('\n--- Test 1: Operator Equals (= / 30-08-2026) ---');
    await setupProcStartCondition('=', '30-08-2026');
    const equalsRows = await page.locator('table tbody tr').allInnerTexts();
    console.log(`Rows returned for Processing Start Equals (30-08-2026): ${equalsRows.length}`);
    ...
    console.log('\n--- Test 2: Operator Before (< 31-12-2026) ---');
    await setupProcStartCondition('<', '31-12-2026');
    ...
    console.log('\n--- Test 3: Operator Between (01-08-2026 to 31-08-2026) ---');
    await setupProcStartCondition('between', '01-08-2026', '31-08-2026');
    ...
    console.log('\n[TEST COMPLETED] Processing Start & End Advanced Filter verified on merdian.olshoperp.com!');
    await context.close();
  });
});
