...
test('Update unit price to 100.000 for SKU in Sales Order 2519646 via UI Crawling', async ({ page }) => {
  ...
  console.log('--- 2. INSPECT DETAIL ROW ON UI ---');
  ...
  const detailRow = page.locator('#SalesOrderDetail tbody tr, table tbody tr').filter({ hasText: new RegExp(sku, 'i') }).first();
  await expect(detailRow, 'Row with SKU ' + sku).toBeVisible({ timeout: 25000 });
  const rowText = await detailRow.innerText();
  console.log('Found detail row text on UI:', rowText);

  // Click edit / update icon on detail row to open modal
  console.log('Clicking edit icon on detail row...');
  const editBtn = detailRow.locator('button#updateButton, button.edit-button, button:has-text("Edit"), a[href*="edit"]').first().or(detailRow.locator('button').first());
  ...
  console.log('--- 3. INPUT UNIT PRICE 100.000 IN MODAL ---');
  ...
  console.log('--- 4. VERIFY BACKEND STATUS & VAT CALCULATION ---');
