...
test.describe('ETM-15447 — TC 4: Single-Active Toggle antar Pill Buttons di Sales Platform', () => {
  const companyCode = 'FAT';

  test('[@TC-SPLG-DRAFT-20260820200504] Verify Single-Active Toggle behavior across Pill Buttons in Sales Platform', async ({ page }) => {
    ...
    console.log('Verified Net Sales < COGS is active exclusively without failed_process in FAT.');
    ...
    console.log('Verified Ready to Process is active exclusively without net_sales_below_cogs in FAT.');

    console.log('[PASS] TC 4 Verification Complete: Single-active toggle functions seamlessly across all pill buttons in Sales Platform (FAT)!');
  });
});
