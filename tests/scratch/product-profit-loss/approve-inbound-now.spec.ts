...
test('Complete & Approve Purchase Inbound IN-5U7ODE9M via Web UI Crawling', async ({ page }) => {
  ...
  console.log('--- 2. UPDATE WAREHOUSE DESTINATION & SUPPLIER VIA UI ---');
  ...
  if (await saveAllBtn.isVisible({ timeout: 5000 }).catch(() => false)) {
    ...
    await page.waitForTimeout(3000);
    console.log('Saved header with valid warehouse & supplier.');
  }

  console.log('--- 3. OPEN AVAILABLE PURCHASE ORDER & USE PO-6A8A5BF6 ---');
  ...
  console.log('--- 4. CLICK APPROVE BUTTON VIA UI ---');
