...
export const MUTATION_INBOUND_DATALIST_PATH = '/supplychain/mutation-inbound';
export const MUTATION_INBOUND_EDIT_PATH_PATTERN =
  /\/supplychain\/mutation-inbound\/edit\/\d+/;

/** Supplier known di lumicharmsid (approved PO). */
export const DEFAULT_INBOUND_SUPPLIER = 'PT. Supplier Lumi 001 Taxable';

/**
 * POM Purchase Inbound klasik (mutation-inbound).
 * Selector: tests/pom-registry/mutation-inbound.yaml
 *
 * Fungsi: GRN — terima barang dari supplier (via PO outstanding) ke Location Destination;
 * approve menambah item stock. Prefix IN*.
 */
export class MutationInboundPage {
  ...
  get supplierCombobox(): Locator {
    return this.multiselect.comboboxByAriaPlaceholder('e.g: PT. ABC Indonesia');
  }
  ...
  async gotoDatalist(): Promise<void> {
    await this.datalist.gotoAndWait(MUTATION_INBOUND_DATALIST_PATH, 'link');
  }
  ...
  async openCreateForm(): Promise<'create' | 'edit'> {
    ...
    const raced = await Promise.race([
      this.page
        .waitForURL(MUTATION_INBOUND_EDIT_PATH_PATTERN, { timeout: 90_000 })
        .then(() => 'edit' as const),
      this.page
        .waitForURL(/\/supplychain\/mutation-inbound\/create$/, {
          timeout: 90_000,
        })
    ...
    const autoEdit = await this.page
      .waitForURL(MUTATION_INBOUND_EDIT_PATH_PATTERN, { timeout: 10_000 })
      .then(() => true)
      .catch(() => false);
    if (autoEdit) {
      await this.expandBasicInformation();
  ...
  async ensureLocationDestinationSelected(): Promise<string> {
    const current = await this.multiselect.selectedLabel(this.locationCombobox);
    if (current && !/^e\.g:|^choose/i.test(current) && current.length > 2) {
      return current;
    }

    await this.multiselect.open(this.locationCombobox);
    await this.page.waitForTimeout(600);
    const option = this.page
      .locator('.multiselect-option:visible')
      .filter({ hasNotText: 'No results found' })
      .first();
    await expect(option, 'Minimal 1 Location Destination').toBeVisible({
      timeout: 25_000,
    });
  ...
  async clickSaveAndNextAndWaitForEdit(): Promise<void> {
    const saveResponse = this.page.waitForResponse(
      (response) => {
        ...
        return (
          pathname === '/api/supplychain/mutation-inbound' ||
          pathname.endsWith('/supplychain/mutation-inbound')
        );
      },
      { timeout: 90_000 },
    );
    ...
    if (!response.ok() || Number(body?.status?.error ?? 0)) {
      throw new Error(
        `Save Purchase Inbound gagal: ${body?.status?.message ?? JSON.stringify(body) ?? `HTTP ${response.status()}`}`,
      );
    }

    await this.page.waitForURL(MUTATION_INBOUND_EDIT_PATH_PATTERN, {
      timeout: 45_000,
    });
  ...
  async openEditFromDatalistByCode(code: string): Promise<void> {
    ...
    await expect(row, `Baris Inbound ${code}`).toBeVisible({
      timeout: 45_000,
    });

    const codeLink = row
      .getByRole('link', { name: code, exact: true })
      .or(row.locator(`a[href*="/supplychain/mutation-inbound/edit/"]`))
      .first();
    ...
    await expect(this.page).toHaveURL(MUTATION_INBOUND_EDIT_PATH_PATTERN, {
      timeout: 45_000,
    });
    await dismissStagingBanner(this.page);
    await this.expandBasicInformation();
  ...
  async clickSaveAllAndWait(): Promise<void> {
    const waitPut = () =>
      this.page.waitForResponse(
        (response) =>
          /\/mutation-inbound\/\d+/.test(response.url()) &&
    ...
    if (response) {
      ...
      if (!response.ok() || Number(body?.status?.error ?? 0)) {
        throw new Error(
          `Update Inbound gagal: ${body?.status?.message ?? `HTTP ${response.status()}`}`,
        );
      }
    }
  ...
  async assertInDatalist(code: string, descriptionSnippet?: string): Promise<void> {
    ...
    const row = this.page.getByRole('row').filter({ hasText: code }).first();
    await expect(row, `Inbound ${code} di datalist`).toBeVisible({
      timeout: 45_000,
    });
  ...
  /** Select Product (outstanding PO) → bulk-fifo. */
  async selectFirstOutstandingProduct(): Promise<string> {
    ...
    const bulkResponse = this.page.waitForResponse(
      (response) =>
        ...
        /mutation-inbound-detail\/bulk-fifo/.test(response.url()),
      { timeout: 90_000 },
    );
    ...
    await expect(option, 'Opsi Select Product (outstanding PO)').toBeVisible({
      timeout: 45_000,
    });
  ...
  async assertDetailHasProduct(skuToken: string): Promise<void> {
    ...
    // Hindari opsi Multiselect Select Product (sering tetap di DOM tapi hidden)
    const tableRow = this.page
      .locator(
        '#InventoryInDetail .p-datatable-tbody tr, #InventoryInDetail tbody tr',
      )
