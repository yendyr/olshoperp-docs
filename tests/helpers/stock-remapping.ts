...
import { getApiUrl, readAuthFromPage, readActiveCompanyFromPage } from './company-access';
import {
  OlshopDatalist,
  OlshopFormActions,
  OlshopMultiselect,
...
export function planSplitQty(rows: AvailableStockRow[]): SplitPlan {
  ...
  const uniqueIds = concatIds.filter((v, i, a) => a.indexOf(v) === i);
  ...
  return {
    qty: total,
    canSplit: false,
    reason: "Stok hanya 1 batch (qty=" + total + "); tidak bisa merepro split ETM-15584.",
    batches,
  };
}

function parseQty(val: string): number | null {
  ...
  if (lastComma >= 0 && lastDot >= 0) {
    ...
    n = last.length === 3 ? Number(parts.join('')) : Number(unsigned.replace(',', '.'));
  } else if (lastDot >= 0) {
    const parts = unsigned.split('.');
    const last = parts[parts.length - 1];
...
export class StockRemappingPage {
  ...
  async readDetailStockIds(): Promise<string[]> {
    ...
    const ids = (text.match(/\b\d{5,}\b/g) ?? []).filter((v, i, a) => a.indexOf(v) === i);
    return ids;
  }
  ...
  async readImportHistorySummary(): Promise<{
    ...
    const trailing = (tableText.match(/\b\d+\b/g) ?? []).map((m) => Number(m));
