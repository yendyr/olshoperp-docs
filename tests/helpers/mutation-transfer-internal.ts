...
export const MUTATION_TRANSFER_INTERNAL_DATALIST_PATH =
  '/supplychain/mutation-transfer-internal';
export const MUTATION_TRANSFER_INTERNAL_EDIT_PATH_PATTERN =
  /\/supplychain\/mutation-transfer-internal\/edit\/\d+/;

/**
 * POM Transfer Internal (mutation-transfer-internal).
 * Selector: tests/pom-registry/mutation-transfer-internal.yaml
 *
 * UI: /supplychain/mutation-transfer-internal · API: supplychain/mutation-transfer
 * Prefix TFI* · Origin + Destination · Select Product → bulk-fifo.
 */
export class MutationTransferInternalPage {
  ...
  async openCreateForm(): Promise<'create' | 'edit'> {
    ...
    const raced = await Promise.race([
      this.page
        .waitForURL(MUTATION_TRANSFER_INTERNAL_EDIT_PATH_PATTERN, {
          timeout: 90_000,
        })
        .then(() => 'edit' as const),
      this.page
        .waitForURL(/\/supplychain\/mutation-transfer-internal\/create$/, {
          timeout: 90_000,
        })
  ...
  async clickSaveAndNextAndWaitForEdit(): Promise<void> {
    const saveResponse = this.page.waitForResponse(
      (response) => {
        ...
        return (
          pathname === '/api/supplychain/mutation-transfer' ||
          pathname.endsWith('/supplychain/mutation-transfer')
        );
      },
      { timeout: 90_000 },
    );
    ...
    if (!response.ok() || Number(body?.status?.error ?? 0)) {
      throw new Error(
        `Save Transfer Internal gagal: ${body?.status?.message ?? JSON.stringify(body) ?? `HTTP ${response.status()}`}`,
      );
    }

    await this.page.waitForURL(MUTATION_TRANSFER_INTERNAL_EDIT_PATH_PATTERN, {
      timeout: 45_000,
    });
  ...
  async openEditFromDatalistByCode(code: string): Promise<void> {
    ...
    await expect(row, `Baris Transfer Internal ${code}`).toBeVisible({
      timeout: 45_000,
    });
    ...
    const codeLink = row
      ...
      .or(
        row.locator(
          `a[href*="/supplychain/mutation-transfer-internal/edit/"]`,
        ),
      )
      .first();
    ...
    await expect(this.page).toHaveURL(
      MUTATION_TRANSFER_INTERNAL_EDIT_PATH_PATTERN,
      { timeout: 45_000 },
    );
    await dismissStagingBanner(this.page);
    await this.expandBasicInformation();
  ...
  async clickSaveAllAndWait(): Promise<void> {
    const waitPut = () =>
      this.page.waitForResponse(
        (response) =>
          /\/mutation-transfer\/\d+/.test(response.url()) &&
    ...
    if (response) {
      ...
      if (!response.ok() || Number(body?.status?.error ?? 0)) {
        throw new Error(
          `Update Transfer Internal gagal: ${body?.status?.message ?? `HTTP ${response.status()}`}`,
        );
      }
    }
  ...
  async assertInDatalist(
    ...
    const row = this.page.getByRole('row').filter({ hasText: code }).first();
    await expect(row, `Transfer Internal ${code} di datalist`).toBeVisible({
      timeout: 45_000,
    });
  ...
  /** Select Product → mutation-transfer-middle-detail/bulk-fifo. */
