...
title: "Create + Approve Purchase Inbound dari approved PO (satu alur)"
summary: "Create inbound dari Available Purchase Order lalu approve dalam satu alur."
status: draft
...
> [!WARNING]
> **Kandidat duplikat — perlu keputusan saat `#renumber-tc`.**
> Alur TC ini = kombinasi **TC-PI-CREATE-001** (create s/d Open) + **TC-PI-APPROVE-001**
> (approve dari show), yang keduanya sudah ada dan sudah direvisi mengikuti UX ≥2026-08.
> Temuan UX dari sesi crawling ini (auto-save draft, Use per-baris, dialog Create Inbound
> Product, Allocate Full Qty) sudah diserap ke `tests/ui-components.md` §10 dan revisi
> TC-PI-CREATE-001. Opsi: (a) hapus TC ini, atau (b) pertahankan sebagai TC gabungan
> create+approve. Rekomendasi: **(a)** — gabungan langkah antar TC sebaiknya jadi flow
> ber-`recalls` (rule 17), bukan TC baru yang menyalin langkah.
>
> Dikonversi dari skema crawler ke format rule 13 pada 2026-08-24; status diturunkan ke
> `draft`/`automated: false` sesuai rule 14 §6B (status `passed` hanya sah dari CLI run).
