...
# Broken (Transfer Scrap) — Requirement Documentation
...
Transfer stok rusak/broken ke virtual warehouse scrap. Wrapper `StockMutationTransferScrapController` delegasi ke `StockMutationTransferController` dengan `process_type = scrap`, kode `TFS`. Kode transaksi prefix **`TFS`**. Filter datalist: `type = tf internal` · `process_type = scrap` · destination `process_group = scrap`.

## 2. Acceptance Criteria (AS-IS)

| ID | Kriteria | Validasi | Fitur |
|----|----------|----------|-------|
| A-01 | User dengan permission `viewAny` melihat datalist | Policy `StockMutationTransferScrap` | List |
...
Origin bisa virtual warehouse (in transit / picked). Destination harus virtual scrap di bawah parent building.

**Alur approve:** POST `mutation-transfer-scrap/{id}/approve` → `StockMutationTransferController@approve` dengan `is_scrap = true`.

## 4. Fitur & Behavior

| ID | Fitur | Trigger | Expected result |
|----|-------|---------|-----------------|
| F-01 | Datalist + filter SearchBuilder | GET `mutation-transfer-scrap` | JSON paginated rows |
| F-02 | Create header | POST `mutation-transfer-scrap` | Row `scm_stock_mutations`, code `TFS` |
| F-03 | Update header | PUT `mutation-transfer-scrap/{id}` | Header updated if `can_update` |
| F-04 | Soft delete | DELETE `mutation-transfer-scrap/{id}` | `deleted_at` set if allowed |
...
Menu terkait: **supplychain-mutation-transfer-external, supplychain-setting**.

## 7. QA Test Notes

- [ ] Create header — validasi tanggal, gudang wajib, kode auto `TFS`
