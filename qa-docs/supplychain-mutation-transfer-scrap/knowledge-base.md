...
# Broken (Transfer Scrap) — Knowledge Base

> **DRAFT** — Dokumen ini adalah draft awal hasil analisis codebase otomatis per 2026-06-19. Perlu direview PM/QA sebelum final.


## 1. Apa itu Broken (Transfer Scrap)?

Transfer stok rusak/broken ke virtual warehouse scrap. Wrapper `StockMutationTransferScrapController` delegasi ke `StockMutationTransferController` dengan `process_type = scrap`, kode `TFS`.

| Item | Nilai |
|------|-------|
| Menu | Supply Chain → Broken (Transfer Scrap) |
| Route UI | `/supplychain/mutation-transfer-scrap` |
| Kode dokumen | `TFS` |
| Tabel header | `scm_stock_mutations` |
| Tabel detail | `scm_transfer_mutation_details` + optional middle details |

**Tujuan:** Memindahkan barang rusak/hilang dari proses fulfillment (checking, transfer external) ke lokasi scrap.
...
### Skenario umum
1. Buka menu **Broken (Transfer Scrap)** → **Create**.
...
```mermaid
flowchart LR
    A["Origin WH"] --> B["Transfer Scrap TFS"]
    B --> C["Scrap virtual WH"]
```
...
A: Lihat: supplychain-mutation-transfer-external, supplychain-setting.

**Q: Bagaimana cara approve?**  
A: POST `mutation-transfer-scrap/{id}/approve` → `StockMutationTransferController@approve` dengan `is_scrap = true`.
