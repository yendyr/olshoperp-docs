...
2. Buat / terima **Sales Invoice**.  
3. **Approve** → piutang + jurnal penjualan.  
...
- [ ] Order **platform** — jangan create manual; tunggu / cek dari Instant Settlement.
...
> Invoice dari platform biasanya tidak bisa di-reject atau dihapus seperti invoice biasa.
...
- **Kalau kamu mau tagih sebagian qty satu SKU** (mis. 5 dari 10) lewat Use, sistem tidak mengizinkan — yang diambil adalah **seluruh sisa** baris itu. Tagih SKU lain di invoice berikutnya.  
...
- **Kalau import Excel**, isi Order Number **atau** Platform Order ID (salah satu); hanya SO internal/general; hasilnya status Open — Approve manual supaya jurnal jalan.  
...
1. Buka Outstanding Sales Order.  
2. Pilih per baris SKU atau per seluruh SO yang masih outstanding.  
3. Qty mengikuti sisa penuh baris — tidak diedit partial di layar ini.  
4. Opsional: tambah Other Cost / Other Discount.
...
2. Klik **Approve**.  
3. Cek bahwa invoice terkunci dan siap ditagih di Account Receive.

### Langkah 4 — Import saldo awal (opsional)

1. Unduh template 3 kolom.  
2. Isi tanggal + Order Number **atau** Platform Order ID.  
3. Upload → SI berstatus Open.  
4. Buka SI → **Approve** agar jurnal terbit.
...
- **Satu SO, dua invoice?** Boleh — bedakan per SKU/line yang di-Use.  
...
- **Import platform?** Tidak boleh — hanya order internal/general.  
...
*Derivatif dari requirement / knowledge-base / technical v2.0 — tanpa menambah fakta di luar sumber.*
