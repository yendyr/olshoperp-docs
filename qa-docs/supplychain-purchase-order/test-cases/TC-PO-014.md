---
doc_type: e2e-test-case
tc_code: TC-PO-014
menu: supplychain-purchase-order
menu_name: "Purchase Order"
test_type: regression
title: "Regresi Urutan Baris Detail Transaksi SCM (LIFO / Last-In-First-Row) - Purchase Order"
summary: "Memastikan baris detail transaksi SCM yang terakhir ditambahkan muncul di baris paling atas (LIFO) pada Purchase Order."
status: draft
owner: QA - Yemima
last_updated: 2026-08-27
requirement_ref: "qa-docs/supplychain-purchase-order/requirement.md"
automated: true
automated_spec: "tests/specs/regression/etm-15214/detail-sorting.spec.ts"
execution_company:
  id: 13
  code: DEV-STG
related_menus: []
preconditions:
  - "User login menggunakan credential E2E: playwright@gmail.com / 12345678."
  - "Company aktif: Dev Staging (DEV-STG, id: 13)."
test_data:
  - field: "Select Product SKU pertama"
    value: "SKU112"
steps:
  - "Buka menu Purchase Order, lalu buka detail transaksi target PO-6A437282."
  - "Tambahkan produk secara berturut-turut: SKU112, SKU66200, BIP-HJOV9, SKU-0708, SKU1, SKU6620."
  - "Amati urutan produk pada tabel detail transaksi."
expected_result: |
  Baris yang terakhir ditambahkan harus muncul di baris paling atas (LIFO). 
  Urutan baris dari atas ke bawah harus: SKU6620 -> SKU1 -> SKU-0708 -> BIP-HJOV9 -> SKU66200 -> SKU112.
test_result:
  status: passed
  started_at: "2026-08-26T00:00:00+07:00"
  finished_at: "2026-08-26T00:05:00+07:00"
  executed_by: "QA - Yemima"
  environment: "staging"
  log_summary: |
    PASS: Urutan baris detail sesuai kriteria LIFO (last-in-first-row).
  report_url: null
test_data_used:
  - "PO-6A437282 | SKU112, SKU66200, BIP-HJOV9, SKU-0708, SKU1, SKU6620"
run_history:
  - run_at: "2026-08-26T00:00:00+07:00"
    status: passed
    executor: "QA - Yemima"
    notes: "Newest selalu di atas."
first_execution:
  at: "2026-08-26"
  via: "tests/specs/regression/etm-15214/detail-sorting.spec.ts"
  jira: "ETM-15214"
last_execution:
  at: "2026-08-26"
  jira: "ETM-15214"
  status: passed
  via: "tests/specs/regression/etm-15214/detail-sorting.spec.ts"
---

# TC-PO-014

## Catatan QA
- **Latar Belakang:** Pengujian dilakukan untuk memverifikasi regresi urutan detail SCM pasca ETM-15214.
- **Relasi JIRA:** Terkait card regresi `ETM-15214`.
