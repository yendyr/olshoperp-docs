...
**Dampak ke menu ini:** Setiap baris SO yang di-settle merujuk **System Product** (`product_id`). Produk **inactive** atau PARENT memblokir approve SO / validasi import. **Product COA Group** (menu terpisah) memetakan SKU ini ke akun jurnal SI/OB saat settlement generate.

**Prasyarat dari menu ini agar settlement lolos:** SKU pada order aktif; stok available pada tanggal settle (`checkShipping`); Product COA Group lengkap untuk setiap produk di baris.

**Independensi:** Ubah harga/master produk **tidak** retroaktif mengubah SI settlement yang sudah generate. Inactive produk setelah upload tidak menghapus batch — cegah dengan validasi pra-upload.

**Detail alur bulk:** [Instant Settlement](../accounting-settlement-upload/requirement.md) · [Product COA Group](../accounting-product-coa-group/requirement.md)
