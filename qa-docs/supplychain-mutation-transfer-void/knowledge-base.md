...
# Void Transfer — Knowledge Base

> **DRAFT** — Dokumen ini adalah draft awal hasil analisis codebase otomatis per 2026-06-19. Perlu direview PM/QA sebelum final.


## 1. Apa itu Void Transfer?

Transfer stok dari virtual warehouse voided order kembali ke gudang fisik. `process_type = from void`, kode `TFV`, origin `process_group = voided_order`.

| Item | Nilai |
|------|-------|
| Menu | Supply Chain → Void Transfer |
| Route UI | `/supplychain/mutation-transfer-void` |
| Kode dokumen | `TFV` |
| Tabel header | `scm_stock_mutations` |
| Tabel detail | `scm_transfer_mutation_details` + middle details |

**Tujuan:** Mengembalikan stok order yang di-void dari lokasi voided order ke gudang operasional.
...
### Skenario umum
1. Buka menu **Void Transfer** → **Create**.
...
```mermaid
flowchart LR
    A["Voided Order WH"] --> B["Transfer Void TFV"]
    B --> C["Physical WH"]
```
...
A: Lihat: sales-order-general, supplychain-cancelled-order.

**Q: Bagaimana cara approve?**  
A: POST `mutation-transfer-void/{id}/approve` — wajib `warehouse_destination` terisi di header sebelum approve.
