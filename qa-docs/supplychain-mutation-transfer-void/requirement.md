...
# Void Transfer — Requirement Documentation
...
Transfer stok dari virtual warehouse voided order kembali ke gudang fisik. `process_type = from void`, kode `TFV`, origin `process_group = voided_order`. Kode transaksi prefix **`TFV`**. Filter datalist: `type = tf internal` · `process_type = from void` · origin voided order virtual WH.

## 2. Acceptance Criteria (AS-IS)

| ID | Kriteria | Validasi | Fitur |
|----|----------|----------|-------|
| A-01 | User dengan permission `viewAny` melihat datalist | Policy `StockMutationTransferVoid` | List |
...
Sering auto-generated dari proses void sales order. Manual create juga didukung.

**Alur approve:** POST `mutation-transfer-void/{id}/approve` — wajib `warehouse_destination` terisi di header sebelum approve.

## 4. Fitur & Behavior

| ID | Fitur | Trigger | Expected result |
|----|-------|---------|-----------------|
| F-01 | Datalist + filter SearchBuilder | GET `mutation-transfer-void` | JSON paginated rows |
| F-02 | Create header | POST `mutation-transfer-void` | Row `scm_stock_mutations`, code `TFV` |
| F-03 | Update header | PUT `mutation-transfer-void/{id}` | Header updated if `can_update` |
| F-04 | Soft delete | DELETE `mutation-transfer-void/{id}` | `deleted_at` set if allowed |
...
Menu terkait: **sales-order-general, supplychain-cancelled-order**.

## 7. QA Test Notes

- [ ] Create header — validasi tanggal, gudang wajib, kode auto `TFV`
