...
# External Transfer — Requirement Documentation
...
Transfer antar building/company dengan fase transit. Subclass `StockMutationTransferExternal`, `type = tf external`, kode `TFE`. Mendukung `transit_status`: `in transit` → `delivered`. Kode transaksi prefix **`TFE`**. Filter datalist: `type = tf external` · `warehouse_origin` and `warehouse_destination` not null · `is_inventory_adjustment = 0`.

## 2. Acceptance Criteria (AS-IS)

| ID | Kriteria | Validasi | Fitur |
|----|----------|----------|-------|
| A-01 | User dengan permission `viewAny` melihat datalist | Policy `StockMutationTransferExternal` | List |
...
Origin warehouse level harus ≥ rack level (config). Tidak bisa reject jika berasal dari sales platform. Dua tahap approval untuk transit.

**Alur approve:** POST `mutation-transfer-external/{id}/approve` — approve pengiriman (in transit) lalu approve penerimaan (`transit` param). Broken qty → scrap warehouse.

## 4. Fitur & Behavior

| ID | Fitur | Trigger | Expected result |
|----|-------|---------|-----------------|
| F-01 | Datalist + filter SearchBuilder | GET `mutation-transfer-external` | JSON paginated rows |
| F-02 | Create header | POST `mutation-transfer-external` | Row `scm_stock_mutations`, code `TFE` |
| F-03 | Update header | PUT `mutation-transfer-external/{id}` | Header updated if `can_update` |
| F-04 | Soft delete | DELETE `mutation-transfer-external/{id}` | `deleted_at` set if allowed |
...
Menu terkait: **supplychain-transfer-inbound, supplychain-mutation-transfer-scrap**.

## 7. QA Test Notes

- [ ] Create header — validasi tanggal, gudang wajib, kode auto `TFE`
