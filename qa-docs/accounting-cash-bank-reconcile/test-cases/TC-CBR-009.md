...
title: 'ACCOUNT RECEIVE CREATE — urutan fiscal dulu, lalu CBR lock (manual)'
summary: 'Account Receive create: D_closed gagal fiscal; D_in + Account A gagal lock + BR.'
status: draft
owner: QA - Yemima
...
steps:
  - "Bagian A: buka /accounting/customer-payment/create. Tanggal = D_closed. Receiving / Cash Bank Account = Account A. Save & Next."
  - "Pesan fiscal, bukan lock CBR."
  - "Bagian B: create baru, tanggal D_in, Account A. Save & Next / Approve."
  - "Pesan lock + BR, bukan fiscal. Account A tetap muncul di pilihan."
expected_result: |
  Urutan validasi wajib di Account Receive (Create manual):
...
1. **Create** Account Receive (`/accounting/customer-payment/create`).
2. Tanggal = D_closed. Cash/Bank receiving = Account A.
3. **Save & Next**. Pesan fiscal §6.4.

### Bagian B — CBR lock
1. Create baru. Tanggal D_in. Account A.
2. **Save & Next** / **Approve**. Pesan lock + BR.
