...
steps:
  - "Bagian A — fiscal dulu: buka /accounting/journal/create. Isi Transaction Date = D_closed. Isi detail Debit/Credit termasuk Cash Bank Account A. Klik Save & Next atau Open."
  - "Catat pesan. Harus pesan fiscal (§6.4). Bukan pesan lock CBR. Journal tidak tersimpan / tidak Open."
  - "Bagian B — CBR lock: buka /accounting/journal/create lagi. Transaction Date = D_in (fiscal Open). Pilih Cash Bank Account A di detail journal."
  - "Save & Next / Open. Harus ditolak pesan lock + kode BR. Bukan pesan fiscal."
  - "Pastikan dropdown/opsi Account A tetap bisa dipilih di tanggal D_in (gagal di validasi simpan, bukan akun hilang)."
expected_result: |
  Urutan validasi wajib di Journal (Create manual):
...
### Bagian A — Fiscal Period gagal (STOP)
1. Buka Journal **Create** (`/accounting/journal/create`).
2. **Transaction Date** = D_closed.
3. Isi 2 baris journal (Debit/Credit) memakai **Cash Bank Account** Account A.
4. **Save & Next** (lalu **Open** jika header sempat tersimpan).
5. Expected: pesan `Fiscal period {date} is already closed.` atau `Date must be in an active fiscal period.` — **bukan** lock CBR.

### Bagian B — Fiscal lolos, CBR lock
1. Create Journal baru. **Transaction Date** = D_in.
2. Detail memakai Account A.
3. **Save & Next** / **Open**.
4. Expected: ditolak lock. Pesan TO-BE menyebut tanggal D_in dan kode BR.
