...
title: 'INSTANT SETTLEMENT APPROVE — fiscal Open, lalu CBR lock by date'
summary: 'Approve Instant Settlement: fiscal Open; Cash/Bank Receiving = Account A; tanggal AR = D_in → gagal lock + BR. Bukan start import.'
status: draft
owner: QA - Yemima
...
preconditions:
  ...
  - "Batch Instant Settlement siap Approve; tanggal settle/AR = D_in (fiscal Open, dalam Period CBR Account A)."
  - "Store Setting Cash/Bank Receiving = Account A (COA locked)."
test_data:
  - field: "UI"
    value: "https://staging.olshoperp.com/accounting/settlement-upload"
  - field: "Cash/Bank Receiving"
    value: "Account A"
  - field: "Tanggal AR"
    value: "D_in"
  - field: "CBR"
    value: "BR-… Approved"
steps:
  - "Buka /accounting/settlement-upload. Pilih batch Import Complete, tanggal AR = D_in, receiving Account A."
  - "Klik Approve. Isi catatan. Konfirmasi Approve."
  - "Harus gagal pesan lock CBR + kode BR (setelah fiscal lolos). Bukan pesan fiscal."
  - "Account Receive tidak terbentuk. FAIL jika AR gagal dengan wording yang tidak menyebut CBR/BR (temuan reopen)."
  - "Tidak menguji lock di start import."
expected_result: |
  Urutan validasi wajib di Instant Settlement (Approve saja):
  ...
  Approve ditolak lock. Pesan TO-BE menyebut tanggal dan kode BR.
  Account Receive tidak terbentuk.
  Bukan gagal di start import.

  [CATATAN QA] Reopen: AR gagal generate, wording belum menyebut CBR/BR — FAIL pesan.
test_result:
  status: not_run
  started_at: null
  finished_at: null
...
1. Batch **Import Complete**, tanggal AR = D_in, **Cash/Bank Receiving** = Account A.
2. **Approve** → harus lock + BR.
3. Cek datalist Account Receive: tidak ada AR baru dari batch ini.
