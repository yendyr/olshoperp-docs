...
title: 'CREDIT NOTE IMPORT — urutan fiscal dulu, lalu CBR lock'
summary: 'Import Credit Note: D_closed gagal fiscal; D_in + Cash/Bank Account A gagal lock di awal import.'
status: draft
owner: QA - Yemima
...
test_data:
  ...
  - field: "UI"
    value: "https://staging.olshoperp.com/accounting/credit-note"
steps:
  - "Datalist Credit Note → Import."
  - "Bagian A: file tanggal D_closed + Cash/Bank Account A → gagal pesan fiscal."
  - "Bagian B: file tanggal D_in + Account A → gagal pesan lock + BR di awal import, bukan pesan fiscal."
expected_result: |
  Urutan validasi wajib di Credit Note (Import):
...
Import CN tanggal D_closed + Account A → pesan fiscal.

### Bagian B — CBR lock
Import CN tanggal D_in + Account A → pesan lock + BR di awal import.
