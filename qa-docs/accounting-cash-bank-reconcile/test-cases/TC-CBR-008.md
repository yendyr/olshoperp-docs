...
title: 'ACCOUNT PAYMENT IMPORT — urutan fiscal dulu, lalu CBR lock'
summary: 'Import Account Payment: D_closed gagal fiscal; D_in + Account A gagal lock di awal import (bukan lolos create lalu gagal Approve).'
status: draft
owner: QA - Yemima
...
test_data:
  ...
  - field: "UI"
    value: "https://staging.olshoperp.com/accounting/supplier-payment"
steps:
  - "Datalist Account Payment → Import."
  - "Bagian A: file dengan tanggal D_closed + Cash Bank Account A. Import gagal pesan fiscal. Tidak ada dokumen AP baru."
  - "Bagian B: file dengan tanggal D_in + Account A. Import gagal pesan lock + BR di awal proses import. Bukan: import sukses lalu gagal di Approve (itu FAIL vs expected ETM-15522)."
expected_result: |
  Urutan validasi wajib di Account Payment (Import):
...
1. Datalist → **Import**.
2. Bank Mutation / sheet memakai tanggal D_closed + Account A.
3. Pesan fiscal. Tidak ada AP ter-create.

### Bagian B — CBR lock (setelah fiscal lolos)
1. **Import** tanggal D_in + Account A.
2. Gagal di **awal import**. Pesan lock + BR.
3. FAIL jika dokumen AP ter-create dan baru gagal saat **Approve** (temuan reopen staging).
